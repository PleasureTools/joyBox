...
export default class ActionMenu extends Mixins(RefsForwarding, EventBus) {
  ...
  private async Delete() {
    if (await this.$confirm.Show('Delete?'))
      this.$rpc.RemoveArchiveRecord(this.record.filename);
  }
  ...
  private async AddObservable() {
    if ((await this.$rpc.AddObservable(this.record.source)).result)
      this.$notification.Show('Added');

    this.close();
  }
  private async RemoveObservable() {
    if ((await this.$rpc.RemoveObservable(this.record.source)))
      this.$notification.Show('Removed');
