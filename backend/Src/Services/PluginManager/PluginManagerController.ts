...
export class PluginManagerController {
  ...
  public AddArbiter(arbiter: Arbiter, id: string): void {
    this.arbiters.set(id, arbiter);
    arbiter.VoteEvent.On(this.voteFn);
  }

  public RemoveArbiter(id: string): void {
  ...
  public Find(id: string): Arbiter | null { return this.arbiters.get(id) ?? null; }

  private voteFn = () => this.Vote();

  private Vote() {
    if ([...this.arbiters.values()].every(x => x.Decision())) { this.manager.Resume(); } else { this.manager.Pause(); }
  }
}
